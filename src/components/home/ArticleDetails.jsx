...
const ArticleDetails = () => {
  ...
  const { related_article, readMore, read_article, moreTag } = useSelector(
    ...
    <>
      <div className="article-details">
        <div className="path">
          ...
          <Link to={`/article/category/${read_article?.category_slug}`}>
        ...
        <div className="auth-time">
          <div className="auth">
            ...
            <span>
              <Link to={`/article/tag/${read_article?.tag_slug}`}>
                {read_article?.tag}
              </Link>
            </span>
          </div>
        ...
        <div className="home-artical-image">
          <img
            src={`http://localhost:3000/articleImages/${read_article?.image}`}
            alt=""
          />
        </div>
        ...
        <div className="more-tags">
          ...
          {moreTag.length > 0 &&
            moreTag.map((teg, index) => (
              <Link to={`/article/tag/${teg}`} key={index}>
                {teg.split('-').join(' ')}
              </Link>
            ))}
        </div>
              ...
              related_article.map((art, index) => (
                ...
                >
                  <img
                    ...
                    alt=""
                  />
                  <span>{convert(art.title.slice(0, 40))}</span>
                </Link>
              ))
            ) : (
