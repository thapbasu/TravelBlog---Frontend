...
export const admin_login = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      'http://localhost:5000/rest-api/admin-login',
      data,
...
export const register = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      'http://localhost:5000/rest-api/user-register',
      data,
      { withCredentials: true }
    );
    dispatch({
...
export const verify_email = (otp) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      'http://localhost:5000/rest-api/verify-email',
      { otp },
      { withCredentials: true }
    );
    localStorage.setItem('blog_token', response.data.token);
...
export const user_login = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      'http://localhost:5000/rest-api/user-login',
      data,
      { withCredentials: true }
    );
    localStorage.setItem('blog_token', response.data.token);
...
export const logout_user =
  ...
  async (dispatch) => {
    try {
      await axios.get('http://localhost:5000/rest-api/logout-user', {
        withCredentials: true,
      });
