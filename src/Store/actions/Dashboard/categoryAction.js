...
export const add_category = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      'http://localhost:5000/rest-api/add-category',
      data,
...
export const get_all_category = (page, searchValue) => async (dispatch) => {
  try {
    const response = await axios.get(
      `http://localhost:5000/rest-api/get-category?page=${page}&&searchValue=${searchValue}`,
      { withCredentials: true }
    );
    dispatch({
...
export const delete_category = (id) => async (dispatch) => {
  try {
    const response = await axios.delete(
      `http://localhost:5000/rest-api/delete-category/${id}`,
      { withCredentials: true }
    );
    dispatch({
...
export const edit_category = (categorySlug) => async (dispatch) => {
  try {
    const response = await axios.get(
      `http://localhost:5000/rest-api/edit-category/${categorySlug}`,
      { withCredentials: true }
    );
    dispatch({
    ...
    dispatch({ type: 'EDIT_REQUEST_SET' });
  } catch (error) {
    console.log(error.response);
  }
};
export const update_category = (id, data) => async (dispatch) => {
  try {
    const response = await axios.patch(
      `http://localhost:5000/rest-api/update-category/${id}`,
      data,
      { withCredentials: true }
    );
    dispatch({
