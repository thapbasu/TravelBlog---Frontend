...
export const dashboard_index_data_get = () => async (dispatch) => {
  try {
    const {
      ...
      'http://localhost:5000/rest-api/get-dashboard-index-data',
...
export const get_notification = (id) => async (dispatch) => {
  try {
    const {
      ...
      `http://localhost:5000/rest-api/get-notification/${id}`,
      { withCredentials: true }
    );
    dispatch({
...
export const seen_notification = (id) => async (dispatch) => {
  try {
    await axios.get(`http://localhost:5000/rest-api/seen-notification/${id}`, {
      withCredentials: true,
    });
  } catch (error) {
...
export const delete_notification = (id) => async (dispatch) => {
  try {
    const {
      ...
      `http://localhost:5000/rest-api/delete-notification/${id}`,
      { withCredentials: true }
    );
    console.log(message);
    dispatch({
...
export const get_users = () => async (dispatch) => {
  try {
    const response = await axios.get(
      'http://localhost:5000/rest-api/get-users',
      { withCredentials: true }
    );
    dispatch({
