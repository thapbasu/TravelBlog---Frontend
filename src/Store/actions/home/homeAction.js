...
export const get_home_tag_category = () => async (dispatch) => {
  try {
    const response = await axios.get(
      'http://localhost:5000/rest-api/home/get-tag-category',
      { withCredentials: true }
    );
    dispatch({
...
export const get_old_recent_acticle = () => async (dispatch) => {
  try {
    const response = await axios.get(
      'http://localhost:5000/rest-api/article/recent-old-get',
      { withCredentials: true }
    );
    dispatch({
...
export const get_popular_article = () => async (dispatch) => {
  try {
    const response = await axios.get(
      'http://localhost:5000/rest-api/article/most-popular-article',
      { withCredentials: true }
    );
    dispatch({
      type: 'GET_POPULAR_REACT_ATICLE_SUCCESS',
      payload: response.data,
    });
  } catch (error) {
    console.log(error);
...
export const get_category_article =
  (categorySlug, currentPage) => async (dispatch) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/rest-api/category-article-get?categorySlug=${categorySlug}&&currentPage=${currentPage}`,
        { withCredentials: true }
      );
      dispatch({
...
export const get_tag_article = (tagSlug, currentPage) => async (dispatch) => {
  try {
    const response = await axios.get(
      `http://localhost:5000/rest-api/tag-article-get?tagSlug=${tagSlug}&&currentPage=${currentPage}`,
      { withCredentials: true }
    );
    dispatch({
...
export const userView = () => async () => {
  try {
    await axios.get(`http://localhost:5000/rest-api/user-view`, {
      withCredentials: true,
    });
  } catch (error) {}
};
